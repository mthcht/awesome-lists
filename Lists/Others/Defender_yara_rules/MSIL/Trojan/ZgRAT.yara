...
rule Trojan_MSIL_ZgRAT_SFT_2147947765_0
{
    meta:
        author = "defender2yara"
        detection_name = "Trojan:MSIL/ZgRAT.SFT!MTB"
        threat_id = "2147947765"
        type = "Trojan"
        platform = "MSIL: .NET intermediate language scripts"
        family = "ZgRAT"
        severity = "Critical"
        ...
        strings_accuracy = "Low"
    strings:
        $x_1_1 = {11 04 06 6f ?? 00 00 0a 11 04 08 6f ?? 00 00 0a 11 04 11 04 6f ?? 00 00 0a 11 04 6f ?? 00 00 0a 6f ?? 00 00 0a 13 05 73 ?? 00 00 0a 13 06 11 06 11 05 17 73 ?? 00 00 0a 13 07 11 07 02 16 02 8e 69 6f ?? 00 00 0a 11 07 6f ?? 00 00 0a 11 06 6f ?? 00 00 0a 13 08}  //weight: 1, accuracy: Low
    condition:
        (filesize < 20MB) and
        (all of ($x*))
}
