...
rule Trojan_MSIL_Darkcloud_PGDK_2147961065_0
{
    meta:
        author = "defender2yara"
        detection_name = "Trojan:MSIL/Darkcloud.PGDK!MTB"
        threat_id = "2147961065"
        type = "Trojan"
        platform = "MSIL: .NET intermediate language scripts"
        family = "Darkcloud"
        severity = "Critical"
        ...
        strings_accuracy = "Low"
    strings:
        $x_5_1 = {06 7b 00 01 00 04 11 06 6f ?? 00 00 0a 11 06 6f ?? 00 00 0a 6f ?? 00 00 0a 13 07 19 8d ?? 00 00 01 25 16 12 07 28 ?? 00 00 0a 9c 25 17 12 07 28 ?? 00 00 0a 9c 25 18 12 07 28 ?? 00 00 0a 9c 7e ?? 00 00 04 25 2d 17}  //weight: 5, accuracy: Low
    condition:
        (filesize < 20MB) and
        (all of ($x*))
}

rule Trojan_MSIL_Darkcloud_ZUC_2147970641_0
{
    meta:
        author = "defender2yara"
        detection_name = "Trojan:MSIL/Darkcloud.ZUC!MTB"
        threat_id = "2147970641"
        type = "Trojan"
        platform = "MSIL: .NET intermediate language scripts"
        family = "Darkcloud"
        severity = "Critical"
        ...
        strings_accuracy = "Low"
    strings:
        $x_10_1 = {1f 13 2b f5 12 05 28 ?? 00 00 0a 74 ?? 00 00 01 28 ?? 00 00 06 13 0a 11 0a 28 ?? 00 00 0a 00 11 0a 0b 1f 10 13 0c 38 ?? fe ff ff 00 14 0b 1f 10 13 0c 38 ?? fd ff ff 07 2a}  //weight: 10, accuracy: Low
    condition:
        (filesize < 20MB) and
        (all of ($x*))
}
