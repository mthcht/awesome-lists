...
rule Trojan_MSIL_AMSIBypass_SXC_2147974131_0
{
    meta:
        author = "defender2yara"
        detection_name = "Trojan:MSIL/AMSIBypass.SXC!MTB"
        threat_id = "2147974131"
        type = "Trojan"
        platform = "MSIL: .NET intermediate language scripts"
        family = "AMSIBypass"
        severity = "Critical"
        ...
        strings_accuracy = "Low"
    strings:
        $x_1_1 = {26 1c 8d 07 00 00 01 25 d0 01 00 00 04 28 ?? 00 00 0a 13 06 08 07 11 06 11 06 8e 69 12 07 28 ?? 00 00 06 26 08 12 03 12 04 11 05 12 05 28 ?? 00 00 06 26}  //weight: 1, accuracy: Low
    condition:
        (filesize < 20MB) and
        (all of ($x*))
}

rule Trojan_MSIL_AMSIBypass_SXD_2147976430_0
{
    meta:
        author = "defender2yara"
        detection_name = "Trojan:MSIL/AMSIBypass.SXD!MTB"
        threat_id = "2147976430"
        type = "Trojan"
        platform = "MSIL: .NET intermediate language scripts"
        family = "AMSIBypass"
        severity = "Critical"
        ...
        strings_accuracy = "Low"
    strings:
        $x_20_1 = {1f 40 12 05 28 0b 00 00 06 26 06 28 0d 00 00 0a 08 6a 58 [0-2] 6a 58 28 0e 00 00 0a [0-2] 28 11 00 00 0a 06}  //weight: 20, accuracy: Low
        $x_5_2 = "AmsiScanBuffer" wide //weight: 5
    condition:
        (filesize < 20MB) and
        (all of ($x*))
}
