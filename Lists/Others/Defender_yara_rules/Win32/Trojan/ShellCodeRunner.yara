...
rule Trojan_Win32_ShellCodeRunner_GTD_2147965362_0
{
    meta:
        author = "defender2yara"
        detection_name = "Trojan:Win32/ShellCodeRunner.GTD!MTB"
        threat_id = "2147965362"
        type = "Trojan"
        platform = "Win32: Windows 32-bit platform"
        family = "ShellCodeRunner"
        severity = "Critical"
        ...
        strings_accuracy = "Low"
    strings:
        $x_5_1 = {6a 40 68 00 30 00 00 8b 0d ?? ?? ?? ?? 51 6a 00 ff 15}  //weight: 5, accuracy: Low
        $x_5_2 = {cc 55 8b ec 83 ec 08 c7 45 fc ?? ?? ?? ?? 8b 45 fc 50 6a 00 6a 00 ff 15}  //weight: 5, accuracy: Low
        $x_1_3 = "NYIRNWRG" ascii //weight: 1
    condition:
        (filesize < 20MB) and
        (all of ($x*))
}

rule Trojan_Win32_ShellCodeRunner_GDK_2147970078_0
{
    meta:
        author = "defender2yara"
        detection_name = "Trojan:Win32/ShellCodeRunner.GDK!MTB"
        threat_id = "2147970078"
        type = "Trojan"
        platform = "Win32: Windows 32-bit platform"
        family = "ShellCodeRunner"
        severity = "Critical"
        ...
        strings_accuracy = "Low"
    strings:
        $x_10_1 = {8b f0 8d 8d ?? ?? ?? ?? e8 ?? ?? ?? ?? 8b c8 8b 85 ?? ?? ?? ?? 33 d2 f7 f1 0f be 14 16 8b 85 ?? ?? ?? ?? 03 85 ?? ?? ?? ?? 0f be 08 33 ca 8b 95 ?? ?? ?? ?? 03 95 ?? ?? ?? ?? 88 0a eb}  //weight: 10, accuracy: Low
    condition:
        (filesize < 20MB) and
        (all of ($x*))
}
