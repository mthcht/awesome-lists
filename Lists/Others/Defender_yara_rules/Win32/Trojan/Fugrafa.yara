...
rule Trojan_Win32_Fugrafa_AHB_2147960228_0
{
    meta:
        author = "defender2yara"
        detection_name = "Trojan:Win32/Fugrafa.AHB!MTB"
        threat_id = "2147960228"
        type = "Trojan"
        platform = "Win32: Windows 32-bit platform"
        family = "Fugrafa"
        severity = "Critical"
        ...
        strings_accuracy = "Low"
    strings:
        $x_30_1 = {0f 10 01 0f 29 85 ?? ff ff ff 0f 28 85 ?? ff ff ff 66 0f ef 85 ?? ff ff ff 0f 29 85 00 ff ff ff 0f 28 85 00 ff ff ff 8b 55 c8}  //weight: 30, accuracy: Low
        $x_20_2 = {8b 4d 94 83 f0 ?? 83 f1 ?? 89 85 78 ff ff ff 89 8d 7c ff ff ff c7 45 88 ?? 00 00 00}  //weight: 20, accuracy: Low
    condition:
        (filesize < 20MB) and
        (all of ($x*))
}

rule Trojan_Win32_Fugrafa_SXA_2147960271_0
{
    meta:
        author = "defender2yara"
        detection_name = "Trojan:Win32/Fugrafa.SXA!MTB"
        threat_id = "2147960271"
        type = "Trojan"
        platform = "Win32: Windows 32-bit platform"
        family = "Fugrafa"
        severity = "Critical"
        ...
        strings_accuracy = "High"
    strings:
        $x_20_1 = {8b 4d 88 8b 55 8c 8b 45 84 03 4d 80 13 d0 83 c1 01 83 d2 00 89 8d 60 ff ff ff 89 95 64 ff ff ff}  //weight: 20, accuracy: High
        $x_10_2 = {0f 28 45 e0 0f 29 85 10 ff ff ff 8b 4d c8 0f 10 01 0f 29 85 20 ff ff ff 0f 28 85 20 ff ff ff 66 0f ef 85 10 ff ff ff 0f 29 85 00 ff ff ff 0f 28 85 00 ff ff ff 8b 55 c8 0f 11 02}  //weight: 10, accuracy: High
    condition:
        (filesize < 20MB) and
        (all of ($x*))
}
