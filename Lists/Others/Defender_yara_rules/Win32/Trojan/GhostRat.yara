...
rule Trojan_Win32_GhostRat_GMX_2147957706_0
{
    meta:
        author = "defender2yara"
        detection_name = "Trojan:Win32/GhostRat.GMX!MTB"
        threat_id = "2147957706"
        type = "Trojan"
        platform = "Win32: Windows 32-bit platform"
        family = "GhostRat"
        severity = "Critical"
        ...
        strings_accuracy = "Low"
    strings:
        $x_10_1 = {6a 40 68 00 10 00 00 57 6a 00 ff 15 ?? ?? ?? ?? 8b d8 89 5d e8 6a 00 85 db}  //weight: 10, accuracy: Low
        $x_1_2 = "fs-im-kefu.7moor-fs1.com" ascii //weight: 1
        $x_1_3 = "\\Documents\\shell.txt" ascii //weight: 1
    condition:
        (filesize < 20MB) and
        (all of ($x*))
}
