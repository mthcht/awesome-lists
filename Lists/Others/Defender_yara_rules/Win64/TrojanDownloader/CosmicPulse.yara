...
rule TrojanDownloader_Win64_CosmicPulse_BE_2147961664_0
{
    meta:
        author = "defender2yara"
        detection_name = "TrojanDownloader:Win64/CosmicPulse.BE!dha"
        threat_id = "2147961664"
        type = "TrojanDownloader"
        platform = "Win64: Windows 64-bit platform"
        family = "CosmicPulse"
        severity = "Critical"
        ...
        strings_accuracy = "High"
    strings:
        $x_1_1 = "(KHTML, like Gecko) Chrome/131.0.0.0.1 Safari/537.36 Edg/133.0.0.0.1 HEAD/" wide //weight: 1
        $x_1_2 = "oledlg.dll.OleUIBusyW" ascii //weight: 1
        $x_1_3 = " ID" wide //weight: 1
        $x_1_4 = "comObj.dll" ascii //weight: 1
    condition:
        (filesize < 20MB) and
        (all of ($x*))
}
