...
rule TrojanDropper_Win64_Dlass_GVC_2147957363_0
{
    meta:
        author = "defender2yara"
        detection_name = "TrojanDropper:Win64/Dlass.GVC!MTB"
        threat_id = "2147957363"
        type = "TrojanDropper"
        platform = "Win64: Windows 64-bit platform"
        family = "Dlass"
        severity = "Critical"
        ...
        strings_accuracy = "High"
    strings:
        $x_2_1 = {72 44 6c 50 74 53 cd e6 d7 7b 0b 2a 01 00 00 00 c9 7c 6e 00 ac ea 6a 00 00 a2 0a 00 6a db df 05 8a 7d 6a 00 00 d4 00 00 98 d8 59 76}  //weight: 2, accuracy: High
    condition:
        (filesize < 20MB) and
        (all of ($x*))
}
