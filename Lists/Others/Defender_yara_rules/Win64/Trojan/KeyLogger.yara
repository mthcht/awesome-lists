...
rule Trojan_Win64_KeyLogger_KK_2147970441_1
{
    meta:
        author = "defender2yara"
        detection_name = "Trojan:Win64/KeyLogger.KK!MTB"
        ...
        strings_accuracy = "High"
    strings:
        $x_5_1 = "%sKeystroke: %s (0x%x)%s" ascii //weight: 5
        $x_4_2 = "%c%c%c%c%c%c%c%c.exe" ascii //weight: 4
        $x_3_3 = "SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\RunOnce" ascii //weight: 3
        $x_2_4 = "GetAsyncKeyState" ascii //weight: 2
        $x_1_5 = "Runtime Broker" ascii //weight: 1
    condition:
        (filesize < 20MB) and
        (all of ($x*))
}

rule Trojan_Win64_KeyLogger_AC_2147973349_0
{
    meta:
        author = "defender2yara"
...
rule Trojan_Win64_KeyLogger_AD_2147973350_0
{
    meta:
        author = "defender2yara"
        ...
        strings_accuracy = "High"
    strings:
        $x_1_1 = "rdmic /shell /livevoice /stoplivemic /screenshare /closeallapps /keylogger" ascii //weight: 1
        $x_1_2 = "Screen sharing started. Sending screenshots every ~2s to this channel." ascii //weight: 1
        $x_1_3 = "Grab all saved passwords from this PC" ascii //weight: 1
        $x_1_4 = "Keylogger is already running" ascii //weight: 1
