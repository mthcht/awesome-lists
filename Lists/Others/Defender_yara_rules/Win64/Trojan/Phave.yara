...
rule Trojan_Win64_Phave_MK_2147965658_0
{
    meta:
        author = "defender2yara"
        detection_name = "Trojan:Win64/Phave.MK!MTB"
        threat_id = "2147965658"
        type = "Trojan"
        platform = "Win64: Windows 64-bit platform"
        family = "Phave"
        severity = "Critical"
        ...
        strings_accuracy = "High"
    strings:
        $x_15_1 = "[+] Enable %ls privilege" ascii //weight: 15
        $x_10_2 = "Failed to enable SeDebugPrivilege. You might not have sufficient rights." ascii //weight: 10
        $x_3_3 = "Failed to write to target process memory. Error: %d" ascii //weight: 3
        $x_2_4 = "This program is running from: %s" ascii //weight: 2
    condition:
        (filesize < 20MB) and
        (all of ($x*))
}
