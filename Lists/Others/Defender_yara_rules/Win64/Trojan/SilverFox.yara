...
rule Trojan_Win64_SilverFox_GXN_2147974379_0
{
    meta:
        author = "defender2yara"
        detection_name = "Trojan:Win64/SilverFox.GXN!MTB"
        threat_id = "2147974379"
        type = "Trojan"
        platform = "Win64: Windows 64-bit platform"
        family = "SilverFox"
        severity = "Critical"
        ...
        strings_accuracy = "High"
    strings:
        $x_5_1 = {4d 8d 40 01 80 f2 13 41 88 50 ff 49 83 e9 01}  //weight: 5, accuracy: High
        $x_5_2 = {48 8d 52 01 80 f1 13 88 4a ff 49 83 e8 01}  //weight: 5, accuracy: High
    condition:
        (filesize < 20MB) and
        (all of ($x*))
}

rule Trojan_Win64_SilverFox_AHA_2147974542_0
{
    meta:
        author = "defender2yara"
        detection_name = "Trojan:Win64/SilverFox.AHA!MTB"
        threat_id = "2147974542"
        type = "Trojan"
        platform = "Win64: Windows 64-bit platform"
        family = "SilverFox"
        severity = "Critical"
        ...
        strings_accuracy = "Low"
    strings:
        $x_30_1 = {41 f7 e0 41 8b c0 41 ff c0 2b c2 d1 e8 03 c2 c1 e8 ?? 6b c0 ?? 2b c8 8a 4c 0c 38 32 0e 48 ff c6 41 88 09 49 ff c1 44 3b c5 72}  //weight: 30, accuracy: Low
    condition:
        (filesize < 20MB) and
        (all of ($x*))
}

rule Trojan_Win64_SilverFox_AH_2147977037_0
{
    meta:
        author = "defender2yara"
        detection_name = "Trojan:Win64/SilverFox.AH!MTB"
        threat_id = "2147977037"
        type = "Trojan"
        platform = "Win64: Windows 64-bit platform"
        family = "SilverFox"
        severity = "Critical"
        ...
        strings_accuracy = "Low"
    strings:
        $x_30_1 = {0f 42 f0 49 33 0c d0 ?? 0f b6 44 d0 10 44 30 d0 40 80 fe ?? 41 0f 93 c3 45 0f b6 4c d0 11 45 30 d1 41 80 f9 ?? 41 0f 94 c1 45 20 d9 45 31 db 3c ?? b8 00 00 00 00 48 0f 44 c1 40 84 f6 66 0f 6e c6}  //weight: 30, accuracy: Low
    condition:
        (filesize < 20MB) and
        (all of ($x*))
}
